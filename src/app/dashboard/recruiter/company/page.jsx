...
const imgbbApiKey = process.env.NEXT_PUBLIC_IMGBB_API_KEY || '24300300df21dbe627c2f6d5cefb458e';
...
export default function RecruiterCompanyPage() {
  ...
  const fetchCompany = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${baseUrl}/api/companies/recruiter/${user.email}`);
      if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (company?._id) {
        ...
        res = await fetch(`${baseUrl}/api/companies`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (res.ok) {
        toast.success(company?._id ? 'Company profile updated successfully' : 'Company registered successfully');
        setIsEditing(false);
        fetchCompany();
      } else {
  ...
  const handleSimulateStatus = async (status) => {
    ...
    try {
      const res = await fetch(`${baseUrl}/api/companies/${company._id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-zinc-400">
        ...
        <p className="text-sm">Please sign in as a recruiter to manage company details.</p>
      </div>
    );
  }
      ...
      <div className="min-h-[70vh] flex items-center justify-center bg-[#0d0d0e] px-4">
        <div className="max-w-xl w-full bg-[#121214] border border-zinc-900 rounded-xl p-12 text-center shadow-2xl space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-xl font-semibold text-white">No Company Profile Registered</h2>
            <p className="text-zinc-400 text-sm leading-relaxed">
              You haven't registered a company profile yet. Please register your company details to start posting and managing jobs on HireLoop.
            </p>
          </div>
          <Button 
          ...
          <Form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Select
                ...
                onSelectionChange={(key) => setFormValues({ ...formValues, industry: key })}
              >
                ...
                <Select.Popover className={popoverClasses}>
                  <ListBox className="outline-none">
                    {industries.map(ind => (
                      <ListBox.Item key={ind.id} id={ind.id} className={listItemClasses} textValue={ind.name}>
                        {ind.name}
                      </ListBox.Item>
                    ))}
                  </ListBox>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              <Select
                ...
                onSelectionChange={(key) => setFormValues({ ...formValues, employeeCount: key })}
              >
                ...
                <Select.Popover className={popoverClasses}>
                  <ListBox className="outline-none">
                    {employeeRanges.map(range => (
                      <ListBox.Item key={range.id} id={range.id} className={listItemClasses} textValue={range.name}>
                        {range.name}
                      </ListBox.Item>
                    ))}
                  </ListBox>
                ...
                {logoUrl ? (
                  <div className="relative flex items-center gap-3 p-3 bg-zinc-900/40 border border-zinc-800 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-zinc-300 font-medium truncate">Logo uploaded</p>
                      <button 
                        type="button" 
                        onClick={removeLogo}
  ...
  if (company.status === 'approved') {
    statusBadgeStyles = 'bg-emerald-950/40 text-emerald-400 border border-emerald-900/50';
    statusBanner = (
      <div className="flex items-start gap-3 p-4 bg-emerald-950/20 border border-emerald-900/50 rounded-xl text-emerald-300 text-sm">
        ...
        <div>
          <span className="font-semibold block text-emerald-200 mb-0.5">Verification Approved</span>
          Your profile is approved. You can post and manage jobs publicly on HireLoop.
        </div>
      </div>
    );
    ...
    statusBanner = (
      <div className="flex items-start gap-3 p-4 bg-red-950/20 border border-red-900/50 rounded-xl text-red-300 text-sm">
        ...
        <div>
          <span className="font-semibold block text-red-200 mb-0.5">Verification Rejected</span>
          Your registration request was rejected by admin. Please review and update your information or contact admin support.
        </div>
      </div>
    );
  } else {
    // Default to 'pending'
    statusBadgeStyles = 'bg-amber-950/40 text-amber-400 border border-amber-900/50';
    statusBanner = (
      <div className="flex items-start gap-3 p-4 bg-amber-950/20 border border-amber-900/50 rounded-xl text-amber-300 text-sm">
        ...
        <div>
          <span className="font-semibold block text-amber-200 mb-0.5">Verification Pending</span>
          Your company profile is under review by admin. You will be allowed to publish jobs once verification is approved.
        </div>
      </div>
    );
  }

  const selectedIndustry = industries.find(ind => ind.id === company.industry)?.name || company.industry;
  const selectedEmployees = employeeRanges.find(range => range.id === company.employeeCount)?.name || `${company.employeeCount} employees`;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <Card className="bg-[#121214] border border-zinc-900 rounded-xl p-8 shadow-2xl">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ${statusBadgeStyles}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 py-6 border-b border-zinc-800/80">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase text-zinc-500 font-semibold tracking-wide block">Website</span>
                <a 
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase text-zinc-500 font-semibold tracking-wide block">Location</span>
                <span className="text-sm text-zinc-300 font-medium">{company.location}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase text-zinc-500 font-semibold tracking-wide block">Industry</span>
                <span className="text-sm text-zinc-300 font-medium capitalize">{selectedIndustry}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase text-zinc-500 font-semibold tracking-wide block">Company Size</span>
                <span className="text-sm text-zinc-300 font-medium">{selectedEmployees}</span>
              </div>
            </div>
          </div>
          ...
          {/* Description */}
          <div className="pt-6">
            <h3 className="text-sm font-semibold text-zinc-400 mb-2.5 uppercase tracking-wider">About the Company</h3>
        ...
        <Card className="bg-[#1c1c1e]/40 border border-amber-900/30 rounded-xl p-6 shadow-md">
          <div className="flex items-center gap-2.5 mb-3 text-amber-400">
            <Eye size={18} />
            <h3 className="text-sm font-semibold uppercase tracking-wider">Simulate Admin Controls (Dev sandbox)</h3>
          </div>
          <p className="text-xs text-zinc-400 mb-4 leading-relaxed">
            Use these controls to instantly toggle your company's status in the database to verify different dashboard states (posting validation, status alerts, badges).
          </p>
